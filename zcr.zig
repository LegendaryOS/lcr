...
fn fetchRepoList(allocator: Allocator) ![]u8 {
    ...
    var child = std.ChildProcess.init(&[_][]const u8{ "curl", "-s", repo_url }, allocator);
...
fn parseRepoList(allocator: Allocator, content: []const u8, package: []const u8) !?[]u8 {
    ...
    while (lines.next()) |line| {
        ...
        if (mem.eql(u8, mem.trim(u8, pkg, " "), package)) {
            return try allocator.dupe(u8, mem.trim(u8, repo, " "));
        }
    }
    return null;
}
...
fn installPackage(allocator: Allocator, package: []const u8) !void {
    ...
    var child = std.ChildProcess.init(&[_][]const u8{ "git", "clone", repo_url, install_dir }, allocator);
    try child.spawn();
    const status = try child.wait();
    if (status != .Exited or status.Exited != 0) {
    ...
    if (fs.cwd().access(unpack_script, .{ .mode = .read_write })) |_| {
        ...
        var exec_child = std.ChildProcess.init(&[_][]const u8{ "sudo", "sh", unpack_script }, allocator);
...
fn removePackage(allocator: Allocator, package: []const u8) !void {
    ...
    if (fs.cwd().access(remove_script, .{ .mode = .read_write })) |_| {
        ...
        var exec_child = std.ChildProcess.init(&[_][]const u8{ "sudo", "sh", remove_script }, allocator);
        try exec_child.spawn();
        const exec_status = try exec_child.wait();
        if (exec_status != .Exited or exec_status.Exited != 0) {
...
fn autoremove(allocator: Allocator) !void {
